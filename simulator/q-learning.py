...
def predict_ticket(s, pxdiff):
    global meta_dict
    search_date = begin_date + dt.timedelta(days=s[0])
    ...
    orders = predict_order(search_date_str, search_list, category_distribution, class_distribution,
...
if __name__ == "__main__":
    ...
    for i in range(0, n_status + 2, 1):
        for j in range(0, tickets + 1, 1):
            Q_table[(i, j)] = [0] * len(actions)
